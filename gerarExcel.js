...
async function gerarExcel() {
  ...
  try {
    ...
    row = addRow(row, 'Tipo de Registro da Empresa:', formData?.page3?.tipoEmpresaRegistro || '');
    ...
    row = addRow(row, 'Houve Projeto de Demolição?', formData?.page5?.projetoDemolicao || '');
    row = addRow(row, 'Profissional:', formData?.page5?.profissionalProjetoDemol || '');
    row = addRow(row, 'CPF:', formData?.page5?.cpfProjetoDemol || '');
    row = addRow(row, 'Conselho:', formData?.page5?.tipoRegistroProjetoDemol === 'CREA' ? 'CREA' : (formData?.page5?.siglaConselhoProjetoDemol || ''));
    row = addRow(row, 'Número do Registro:', formData?.page5?.numeroCreaProjetoDemol || formData?.page5?.numeroConselhoProjetoDemol || '');
    row = addRow(row, 'ART ou Outro Tipo?', formData?.page5?.tipoArtProjetoDemol === 'ART' ? 'ART' : (formData?.page5?.nomeOutrosArtProjetoDemol || ''));
    row = addRow(row, 'Número:', formData?.page5?.artNumeroProjetoDemol || formData?.page5?.numeroOutrosArtProjetoDemol || '');

    row = addSection('EXECUÇÃO DA DEMOLIÇÃO', row);
    row = addRow(row, 'Foi Executada a Demolição?', formData?.page5?.execucaoDemolicao || '');
    row = addRow(row, 'Profissional:', formData?.page5?.profissionalExecDemol || '');
    row = addRow(row, 'CPF:', formData?.page5?.cpfExecDemol || '');
    row = addRow(row, 'Conselho:', formData?.page5?.tipoRegistroExecDemol === 'CREA' ? 'CREA' : (formData?.page5?.siglaConselhoExecDemol || ''));
    row = addRow(row, 'Número do Registro:', formData?.page5?.numeroCreaExecDemol || formData?.page5?.numeroConselhoExecDemol || '');
    row = addRow(row, 'ART ou Outro Tipo?', formData?.page5?.tipoArtExecDemol === 'ART' ? 'ART' : (formData?.page5?.nomeOutrosArtExecDemol || ''));
    row = addRow(row, 'Número:', formData?.page5?.artNumeroExecDemol || formData?.page5?.numeroOutrosArtExecDemol || '');   
